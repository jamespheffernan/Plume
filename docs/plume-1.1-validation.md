...
- **TestFlight delivery confirmed:** Plume **1.1.0 (202609241719)** is VALID / IN_BETA_TESTING and available to James's existing internal group. See [delivery receipt](plume-1.1-testflight.md).
- OmniFocus updated and read back: action `aNjYuI7OzCq` is **Open TestFlight on my iPhone and test Plume 1.1**, tagged Phone, with no dates. Its notes contain download instructions, the exact build and this checklist. The notification-threading change is included. The project remains open; the physical-iPhone checks have not been performed.
